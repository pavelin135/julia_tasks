...
function find_internal_border!(robot)
    ...
    function border(robot, side, last_side::Bool)# last_side -- определяет, был ли робот под границей одним шагом назад, 0 - если был под границей, 1 - если не был
        while isborder(robot, Nord)
            move!(robot, side)
            last_side = false
        end
        if !isborder(robot, side) && !last_side
            move!(robot, side)
            border(robot, side, true)
        end
    end
