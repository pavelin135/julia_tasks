...
function check_coord(robot::CoordsRobot) # проверяет, находится ли робот выше текущей макс точки
    if find_border(robot) === nothing
        return
    end
    if robot.max_coord[2] < get_coord(robot)[2]
        robot.max_coord = (robot.coord.X, robot.coord.Y)
    end
end
...
function start(robot)
    ...
    N_robot = CoordsRobot(robot, Coordinates(0, 0), left(side), left(side), side, (0,0))
    surround(N_robot)
    return N_robot.max_coord
end